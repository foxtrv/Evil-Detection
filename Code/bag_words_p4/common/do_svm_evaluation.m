...
% SVMModel = fitcsvm(xTest, labels,'KernelFunction','RBF','KernelScale','auto',...
%     'ClassNames',[1, 0]);
%%% Compute ROC and RPC on training data
[predictions, score2]= predict(svmmodel, xTest);
% predictions = predictions';
% correctLabels = sum(predictions == labels);
values = score2(:,1);%-score2(:,2);
...
values = score2(:,1);%-score2(:,2);%[Pc_d_pos_test(1,:)-Pc_d_pos_test(2,:) , Pc_d_neg_test(1,:)-Pc_d_neg_test(2,:)];
...
%% read in user face.. in this case: mine
im = imread('trevor.png');
    ...
    %%% Resize image, proved Preprocessing.Image_Size isn't zero in which
    %%% case, do nothing.
...
%     fprintf('Points %d = %d\n', i, nPoints);
    ...
    for p = 1:nPoints
      for c = 1:VQ.Codebook_Size
        distance(p,c) = norm(centers(:,c) - double(d(:,p)));
      end
    end
...
%    save(ip_file_names{i},'descriptor_vq','histg','-append');
  [predictions2, score2]= predict(svmmodel, histg);
    if(predictions2 == 0)
        fprintf('Not a criminal\n')
    else
        fprintf('Criminal\n')
    end
